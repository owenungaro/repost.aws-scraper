...
def relaxed_policy_search(text):
    ...
    text = text.lower()
    return bool(re.search(r'"?effect"?\s*:\s*"?allow"?', text) and re.search(r'"?action"?\s*:\s*["{\[]', text))

# Filteres "relaxed" posts: no accepted answer, and loosely detected policy (no overlap with broken)
def filter_relaxed(saved_dir='saved_pages', filtered_dir='filtered_pages/relaxed', broken_dir='filtered_pages/broken'):
    # Checks for broken folder to avoid overlapping
    if not os.path.exists(os.path.join(broken_dir, 'original_policy')):
        print("[ERROR] Broken folder not found. Please run with -b first to ensure relaxed results are unique.")
        return
